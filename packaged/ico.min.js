...
Ico.SparkBar=Class.create(Ico.SparkLine,{calculateStep:function(){return this.options.width/this.data.length},drawLines:function(a,b){var c=this.options.bargraph_lastcolour,d=this.step>2?this.step-1:this.step,e=d/2,g=this.normalise(0);b.each(function(f,h){var j;h=c&&h===b.length-1?c:a;j=this.paper.path().attr({stroke:h,"stroke-width":d});j.moveTo(e,this.options.height-f);j.lineTo(e,this.options.height-g);if(f<g)j.attr({stroke:this.options.bargraph_negativecolour||h});e+=this.step}.bind(this))},showHighlight:function(){}});
Ico.SparkArea=Class.create(Ico.SparkLine,{drawLines:function(a,b){var c=a,d=a,e=0.2,g=this.normalise(0);if(typeof a=="object"){c=a[1];d=a[0];e=1}var f=this.paper.path().attr({fill:c,stroke:c,"stroke-width":0,"stroke-opacity":0,opacity:e}).moveTo(0,this.options.height-g).lineTo(0,this.options.height-b.first()),h=this.paper.path().attr({stroke:d,"stroke-width":this.options.stroke_width}).moveTo(0,this.options.height-b.first()),j=0;b.slice(1).each(function(i){j+=this.step;f.lineTo(j,this.options.height-
