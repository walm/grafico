...
b.attrs.y+c.height-this.options.height;d.translate(0,a*-1-1);e&&e.translate(0,a+1)}if(b.attrs.x<0){a=b.attrs.x;d.translate(a*-1+1,0);f&&f.translate(a-1,0);e&&e.translate(a-1,0)}if(b.attrs.x+c.width>this.options.width){a=b.attrs.x+c.width-this.options.width;d.translate(a*-1-1,0);f&&f.translate(a+1,0);e&&e.translate(a+1,0)}}},drawNib:function(a,b,c){return this.paper.path().attr({fill:this.options.label_colour,opacity:1,stroke:this.options.label_colour,"stroke-width":"1px"}).moveTo(a.attrs.x-c,a.attrs.y+
b.height/2+c-1.5).lineTo(a.attrs.x,a.attrs.y+b.height/2+c*2-1.5).lineTo(a.attrs.x+c,a.attrs.y+b.height/2+c-1.5).andClose()},drawRoundRect:function(a,b,c){return this.paper.rect(a.attrs.x-b.width/2-c,a.attrs.y-b.height/2-c,b.width+c*2,b.height+c*2,c*1.5).attr({fill:this.options.label_colour,opacity:1,stroke:this.options.label_colour,"stroke-width":0})}});Array.prototype.sum=function(){for(var a=0,b=0;a<this.length;b+=this[a++]);return b};
...
Ico.SparkBar=Class.create(Ico.SparkLine,{calculateStep:function(){return this.options.width/this.data.length},drawLines:function(a,b){var c=this.options.bargraph_lastcolour,d=this.step>2?this.step-1:this.step,e=d/2,f=this.normalise(0);b.each(function(g,h){var j;h=c&&h===b.length-1?c:a;j=this.paper.path().attr({stroke:h,"stroke-width":d});j.moveTo(e,this.options.height-g);j.lineTo(e,this.options.height-f);if(g<f)j.attr({stroke:this.options.bargraph_negativecolour||h});e+=this.step}.bind(this))},showHighlight:function(){}});
Ico.SparkArea=Class.create(Ico.SparkLine,{drawLines:function(a,b){var c=a,d=a,e=0.2,f=this.normalise(0);if(typeof a=="object"){c=a[1];d=a[0];e=1}var g=this.paper.path().attr({fill:c,stroke:c,"stroke-width":0,"stroke-opacity":0,opacity:e}).moveTo(0,this.options.height-f).lineTo(0,this.options.height-b.first()),h=this.paper.path().attr({stroke:d,"stroke-width":this.options.stroke_width}).moveTo(0,this.options.height-b.first()),j=0;b.slice(1).each(function(i){j+=this.step;g.lineTo(j,this.options.height-
