...
this.normalise(this.options.acceptable_range[1]),this.options.width,this.normalise(this.options.acceptable_range[0])):(this.background=this.paper.rect(0,0,this.options.width,this.options.height));this.background.attr({fill:this.options.background_color,stroke:"none"});this.draw()},calculateStep:function(){return this.options.width/(this.data.length-1)},makeRandomColour:function(){return Raphael.hsb2rgb(Math.random(),1,0.75).hex},normalise:function(a){a-=this.data.min()<0?this.data.min():0;return this.options.height/
(this.data.max()-this.data.min())*a},draw:function(){var a=this.normaliseData(this.data),b;if(this.options.zeroline&&this.data.min()<0){this.options.zeroline=this.options.zeroline===true?{"stroke-width":"1px",stroke:"#BBBBBB"}:this.options.zeroline;b=parseInt(this.options.height-this.normalise(0),10);this.paper.path().attr(this.options.zeroline).moveTo(0,b).lineTo(this.options.width,b)}this.drawLines(this.options.color,a);this.options.highlight&&this.showHighlight(a)},drawLines:function(a,b){var c=
this.paper.path().attr({stroke:a,"stroke-width":this.options.stroke_width}).moveTo(0,this.options.height-b.first()),d=0,e=this.data.min()<0?this.options.stroke_width:0;b.slice(1).each(function(f){d+=this.step;c.lineTo(d,this.options.height-f-e)}.bind(this))},showHighlight:function(a){var b=1+this.options.stroke_width/2,c=this.options.width-b;a=a[this.options.highlight.index||a.length-1]+(b/2).round();var d=this.options.highlight.color||"#f00";if(typeof this.options.highlight.index!=="undefined")c=
