var Grafico={Version:"0.9",Base:{},BaseGraph:{},Normaliser:{},LineGraph:{},AreaGraph:{},StackGraph:{},BarGraph:{},HorizontalBarGraph:{},SparkLine:{},SparkBar:{}},Ico=Grafico;
...
Grafico.BaseGraph=Class.create(Grafico.Base,{initialize:function(a,b,c){this.element=a;this.data_sets=Object.isArray(b)?new Hash({one:b}):$H(b);if(this.chartDefaults().stacked===true){this.real_data=this.deepCopy(this.data_sets);this.stackData(this.data_sets)}this.flat_data=this.data_sets.collect(function(d){return d[1]}).flatten();this.normaliser=new Grafico.Normaliser(this.flat_data,this.normaliserOptions());this.label_step=this.normaliser.step;this.range=this.normaliser.range;this.start_value=
this.normaliser.start_value;this.zero_value=this.normaliser.zero_value;this.data_size=this.longestDataSetLength();if(c&&c.color){c.colors={};this.data_sets.keys().each(function(d){c.colors[d]=c.color})}this.options={width:parseInt(a.getStyle("width"),10),height:parseInt(a.getStyle("height"),10),labels:$A($R(1,this.data_size)),grid:true,plot_padding:10,font_size:10,show_horizontal_labels:true,show_vertical_labels:true,vertical_label_unit:"",colors:this.makeRandomColours(),background_color:a.getStyle("backgroundColor"),
label_color:"#000",grid_color:"#ccc",hover_text_color:"#fff",markers:false,marker_size:5,meanline:false,padding_top:20,draw_axis:true,datalabels:"",hover_color:"",watermark:false,watermark_location:false,hide_empty_label_grid:false,left_padding:false,label_rotation:0};Object.extend(this.options,this.chartDefaults()||{});Object.extend(this.options,c||{});this.x_padding_left=10+this.paddingLeftOffset();this.x_padding_left+=this.options.vertical_label_unit?6:0;this.x_padding_left=this.options.left_padding?
...
this.normaliser.step<this.normaliser.max)this.y_label_count+=1;this.value_labels=this.makeValueLabels(this.y_label_count);this.top_value=this.value_labels.last();this.grid_start_offset=-1;this.paper=new Raphael(this.element,this.options.width,this.options.height);this.background=this.paper.rect(this.x_padding_left,this.y_padding_top,this.graph_width,this.graph_height);this.background.attr({fill:this.options.background_color,stroke:"none"});this.options.meanline=this.options.meanline===true?{"stroke-width":"2px",
...
10),this.y_padding_top);a.lineTo(parseInt(b,10),this.y_padding_top+this.graph_height)}b+=this.step}},drawLines:function(a,b,c,d,e,f){var g=this.calculateCoords(c),h=this.graph_height+this.y_padding_top,j,i,k;if(this.options.start_at_zero===false){k=0;$A(g).each(function(l){l[1]===h&&k++});this.options.odd_horizontal_offset=k;this.options.odd_horizontal_offset>1&&g.splice(0,this.options.odd_horizontal_offset)}if(this.options.stacked_fill||this.options.area){if(this.options.area){a=this.options.area_opacity?
this.options.area_opacity:1.5/this.data_sets.collect(function(l){return l.length}).length;j=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0",opacity:a,"stroke-opacity":0})}else j=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0"});g.unshift([g[0][0],h]);g.push([g[g.length-1][0],h])}else j=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});this.options.datalabels&&this.drawHover(j,d,e,b);$A(g).each(function(l,m){this.drawPlot(m,j,l[0],l[1],b,g,d,e,f)}.bind(this));
if(this.options.area&&this.options.stroke_width>0){i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(l,m){this.drawPlot(m,i,l[0],l[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(i)}},calculateCoords:function(a){var b=this.x_padding_left+this.options.plot_padding-this.step,c=this.graph_height+this.y_padding_top+this.normalise(this.start_value);return $A(a).collect(function(d){d=c-d;b+=this.step;return[b,d]}.bind(this))},
drawFocusHint:function(){var a=this.x_padding_left+2.5-1,b=this.options.height-this.y_padding_bottom,c=this.paper.path().attr({stroke:this.options.label_color,"stroke-width":2});c.moveTo(a,b);c.lineTo(a-5,b-5);c.moveTo(a,b-5);c.lineTo(a-5,b-10)},drawMeanLine:function(a){var b=this.paper.path().attr(this.options.meanline);a=$A(a).inject(0,function(c,d){return d+c})/a.length-0.5;a=this.options.bar?a+this.zero_value*(this.graph_height/this.y_label_count):a;b.moveTo(this.x_padding_left-1,this.options.height-
this.y_padding_bottom-a).lineTo(this.graph_width+this.x_padding_left,this.options.height-this.y_padding_bottom-a)},drawAxis:function(){var a=this.paper.path().attr({stroke:this.options.label_color});a.moveTo(parseInt(this.x_padding_left,10)-0.5,this.options.height-parseInt(this.y_padding_bottom,10)+0.5);a.lineTo(parseInt(this.graph_width+this.x_padding_left,10)-0.5,this.options.height-parseInt(this.y_padding_bottom,10)+0.5);a.moveTo(parseInt(this.x_padding_left,10)-0.5,parseInt(this.options.height-
this.y_padding_bottom,10)+0.5);a.lineTo(parseInt(this.x_padding_left,10)-0.5,parseInt(this.y_padding_top,10))},makeValueLabels:function(a){for(var b=this.label_step,c=this.start_value,d=[],e=0;e<a;e++){c=this.roundValue(c+b,3);d.push(c)}return d},drawMarkers:function(a,b,c,d,e,f){function g(m){return m*b[0]}function h(m){return m*b[1]}var j=parseInt(this.x_padding_left,10)-0.5+g(d),i=this.options.height-this.y_padding_bottom+h(d),k=this.paper.path().attr({stroke:this.options.label_color}),l={font:this.options.font_size+
'px "Arial"',stroke:"none",fill:this.options.label_color};Object.extend(l,f||{});a.each(function(m){if(this.options.draw_axis&&(this.options.hide_empty_label_grid===true&&m!==""||this.options.hide_empty_label_grid===false)){k.moveTo(parseInt(j,10),parseInt(i,10)+0.5);k.lineTo(parseInt(j,10)+h(5),parseInt(i,10)+0.5+g(5))}this.paper.text(j+e[0],i-2-e[1],m.toString()).attr(l).toFront();j+=g(c);i+=h(c)}.bind(this))},drawVerticalLabels:function(){for(var a=this.graph_height/this.y_label_count,b=this.options.vertical_label_unit?
" "+this.options.vertical_label_unit:"",c=0;c<this.value_labels.length;c++)this.value_labels[c]+=b;this.drawMarkers(this.value_labels,[0,-1],a,a,[-8,-2],{"text-anchor":"end"})},drawHorizontalLabels:function(){this.drawMarkers(this.options.labels,[1,0],this.step,this.options.plot_padding,[0,(this.options.font_size+7)*-1],this.options.label_rotation?{rotation:this.options.label_rotation,translation:-this.options.font_size+" 0"}:{})},drawHover:function(a,b,c,d){var e=this.options.stacked_fill||this.options.area?
"fill":"stroke",f=this.options.hover_color||d,g=this.paper.set();b=this.paper.text(a.attrs.x,a.attrs.y-this.options.font_size*1.5-4,b).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var h=b.getBBox(),j=this.drawRoundRect(b,h,4);b.toFront();g.push(j,b).attr({opacity:0}).toFront();this.checkHoverPos({rect:j,set:g});this.globalHoverSet.push(g);a.node.onmouseover=function(i){e==="fill"?a.animate({fill:f,stroke:f},200):a.animate({stroke:f},200);i=this.getMousePos(i);
g[0].attr({x:i.x-h.width/2-4-c.offsetLeft,y:i.y-h.height/2-this.options.font_size*1.5-4-c.offsetTop,opacity:1});g[1].attr({x:i.x-c.offsetLeft,y:i.y-this.options.font_size*1.5-c.offsetTop,opacity:1});a.node.onmousemove=function(k){k=this.getMousePos(k);g[0].attr({x:k.x-h.width/2-4-c.offsetLeft,y:k.y-h.height/2-this.options.font_size*1.5-4-c.offsetTop,opacity:1});g[1].attr({x:k.x-c.offsetLeft,y:k.y-this.options.font_size*1.5-c.offsetTop,opacity:1});this.checkHoverPos(j,g)}.bind(this)}.bind(this);a.node.onmouseout=
function(){e==="fill"?a.animate({fill:d,stroke:d},200):a.animate({stroke:d},200);g.attr({opacity:0})}},checkHoverPos:function(a){var b,c,d,e,f,g;if(a.rect){b=a.rect;c=b.getBBox()}if(a.set)d=a.set;if(a.marker)e=a.marker;if(a.nib)f=a.nib;if(a.textpadding)g=a.textpadding;if(b&&d){if(b.attrs.y<0)if(f&&e){d.translate(0,d.getBBox().height+g*2);e.translate(0,-d.getBBox().height-g*2);f.translate(0,-c.height-g+1.5).scale(1,-1)}else{a=b.attrs.y;d.translate(0,1+a*-1)}if(b.attrs.y+c.height>this.options.height){a=
b.attrs.y+c.height-this.options.height;d.translate(0,a*-1-1);e&&e.translate(0,a+1)}if(b.attrs.x<0){a=b.attrs.x;d.translate(a*-1+1,0);f&&f.translate(a-1,0);e&&e.translate(a-1,0)}if(b.attrs.x+c.width>this.options.width){a=b.attrs.x+c.width-this.options.width;d.translate(a*-1-1,0);f&&f.translate(a+1,0);e&&e.translate(a+1,0)}}},drawNib:function(a,b,c){return this.paper.path().attr({fill:this.options.label_color,opacity:1,stroke:this.options.label_color,"stroke-width":"0px"}).moveTo(a.attrs.x-c,a.attrs.y+
...
h,j);if(a===0)return this.startPlot(b,c-0.5,d,e);this.options.curve_amount?b.cplineTo(c,d,this.options.curve_amount):b.lineTo(c,d)},drawGraphMarkers:function(a,b,c,d){var e=this.paper.circle(b,c,this.options.marker_size),f=this.options.marker_size,g=this.options.hover_color||d,h;e.attr({"stroke-width":"1px",stroke:this.options.background_color,fill:d});this.globalMarkerSet.push(e);e.node.onmouseover=function(){h=parseInt(1.7*f,10);e.animate({r:h,fill:g},200)}.bind(this);e.node.onmouseout=function(){e.animate({r:f,
fill:d},200)}},drawGraphValueMarkers:function(a,b,c,d,e,f,g){if(this.options.odd_horizontal_offset>1)a+=this.options.odd_horizontal_offset;a-=this.options.stacked_fill||this.options.area?1:0;if(f=(this.options.stacked?this.real_data:this.data_sets).collect(function(l){return l[1][a]})[g]){f=""+f.toString().split(".");if(f[1])f[1]=f[1].truncate(3,"")}if(this.options.line||this.options.stacked||(this.options.stacked_fill||this.options.area)&&a!=-1&&typeof f!="undefined"){g=b-this.step/2;var h=this.options.stacked?
c-this.graph_height/18:c-this.graph_height/6,j=this.step,i=this.options.stacked?this.graph_height/9:this.graph_height/3;b=this.paper.circle(b,c,this.options.marker_size);c=this.paper.rect(g,h,j,i);b.attr({"stroke-width":"1px",stroke:this.options.background_color,fill:d,opacity:0});c.attr({fill:d,"stroke-width":0,stroke:d,opacity:0}).toFront();e=this.options.datalabels?e+": "+f:f.toString();e+=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:"";var k=this.paper.set();d=this.paper.text(b.attrs.cx,
b.attrs.cy-this.options.font_size*1.5-8,e).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});f=d.getBBox();e=this.drawRoundRect(d,f,4);f=this.drawNib(d,f,4);d.toFront();k.push(b,e,f,d).attr({opacity:0}).toFront();this.checkHoverPos({rect:e,set:k,marker:b,nib:f,textpadding:4});this.globalHoverSet.push(k);this.globalBlockSet.push(c);c.node.onmouseover=function(){k.animate({opacity:1},200)};c.node.onmouseout=function(){k.animate({opacity:0},200)}}}});
Grafico.AreaGraph=Class.create(Grafico.LineGraph,{chartDefaults:function(){return{area:true,area_opacity:false,stroke_width:0}},setChartSpecificOptions:function(){if(typeof this.options.curve_amount==="undefined")this.options.curve_amount=10},drawPlot:function(a,b,c,d,e,f,g,h,j,i){var k=this.options.area||this.options.stacked_fill;if(!i)if(k===true){if(a!==0&&a!==f.length-1)if(this.options.markers==="circle")this.drawGraphMarkers(a,c,d,e,g,h);else this.options.markers==="value"&&this.drawGraphValueMarkers(a,
c,d,e,g,h,j)}else if(this.options.markers==="circle")this.drawGraphMarkers(a,c,d,e,g,h);else this.options.markers==="value"&&this.drawGraphValueMarkers(a,c,d,e,g,h,j);c-=0.5;if(a===0)return this.startPlot(b,c,d,e);if(this.options.curve_amount&&a>1&&a<f.length-1)b.cplineTo(c,d,this.options.curve_amount);else this.options.curve_amount&&!k&&1?b.cplineTo(c,d,this.options.curve_amount):b.lineTo(c,d)}});
Grafico.StackGraph=Class.create(Grafico.AreaGraph,{chartDefaults:function(){return{stacked:true,stacked_fill:true,stroke_width:5}},normaliserOptions:function(){},stackData:function(a){this.stacked_data=a.collect(function(c){return c[1]});this.stacked_data.reverse();for(a=1;a<this.stacked_data.length;a++)for(var b=0;b<this.stacked_data[0].length;b++)this.stacked_data[a][b]+=this.stacked_data[a-1][b];this.stacked_data.reverse();return this.stacked_data}});
Grafico.BarGraph=Class.create(Grafico.BaseGraph,{chartDefaults:function(){return{bar:true,plot_padding:0,bargraph_lastcolour:false,bargraph_negativecolour:false}},normaliserOptions:function(){return{start_value:0,bar:true}},setChartSpecificOptions:function(){this.bar_padding=5;this.bar_width=this.calculateBarWidth();this.options.plot_padding=this.bar_width/2;this.step=this.calculateStep();this.grid_start_offset=this.bar_padding-1},calculateBarWidth:function(){return this.graph_width/this.data_size-
this.bar_padding},calculateStep:function(){this.data_size=this.data_size===1?2:this.data_size;return(this.graph_width-this.options.plot_padding*2-this.bar_padding*2)/(this.data_size-1)},drawPlot:function(a,b,c,d,e,f,g){b=this.options.height-this.y_padding_bottom-this.zero_value*(this.graph_height/this.y_label_count);var h=this.options.bargraph_lastcolour,j=this.options.bargraph_negativecolour;c+=this.bar_padding;d=this.options.height-this.y_padding_bottom-d-this.zero_value*(this.graph_height/this.y_label_count);
e=h&&a===f.length-1?h:d<0?j:e;f=this.paper.rect(c-this.bar_width/2,b,this.bar_width,d).attr({fill:e,"stroke-width":0,stroke:e});d<0?f.attr({height:-f.attrs.height}):f.translate(0,-d);this.options.datalabels&&this.drawGraphValueMarkers(c,a,f,g,e);this.options.count++},drawHorizontalLabels:function(){this.drawMarkers(this.options.labels,[1,0],this.step,this.bar_padding+this.options.plot_padding,[0,(this.options.font_size+7)*-1],this.options.label_rotation?{rotation:this.options.label_rotation,translation:-this.options.font_size+
" 0"}:{})},drawGrid:function(){var a=this.paper.path().attr({stroke:this.options.grid_color}),b=this.graph_height+this.y_padding_top,c,d;if(this.options.horizontalbar){a.moveTo(this.x_padding_left-0.5,parseInt(b,10)+0.5);a.lineTo(this.x_padding_left+this.graph_width-0.5,parseInt(b,10)+0.5);b-=this.graph_height;a.moveTo(this.x_padding_left-0.5,parseInt(b,10)+0.5);a.lineTo(this.x_padding_left+this.graph_width-0.5,parseInt(b,10)+0.5)}else for(var e=0;e<this.y_label_count+1;e++){a.moveTo(this.x_padding_left-
0.5,parseInt(b,10)+0.5);a.lineTo(this.x_padding_left+this.graph_width-0.5,parseInt(b,10)+0.5);b-=this.graph_height/this.y_label_count}if(this.options.horizontalbar){b=this.x_padding_left+this.options.plot_padding+this.grid_start_offset;c=this.y_label_count;d=this.options.horizontalbar?this.graph_width/this.y_label_count:this.step;for(e=0;e<c;e++){if(this.options.hide_empty_label_grid===true&&this.options.labels[e]!==""||this.options.hide_empty_label_grid===false){a.moveTo(parseInt(b,10),this.y_padding_top);
a.lineTo(parseInt(b,10),this.y_padding_top+this.graph_height)}b+=d}}a.moveTo(parseInt(this.x_padding_left,10)-0.5,this.y_padding_top);a.lineTo(parseInt(this.x_padding_left,10)-0.5,this.y_padding_top+this.graph_height);a.moveTo(parseInt(this.x_padding_left+this.graph_width,10)-0.5,this.y_padding_top);a.lineTo(parseInt(this.x_padding_left+this.graph_width,10)-0.5,this.y_padding_top+this.graph_height)},drawGraphValueMarkers:function(a,b,c,d,e){var f=this.options.hover_color||e,g=this.paper.set();a=this.paper.rect(a-
this.bar_width/2,this.y_padding_top,this.bar_width,this.options.height);d=d[b].toString();b=this.paper.text(c.attrs.x+this.bar_width/2,c.attrs.y-this.options.font_size*1.5,d);a.attr({fill:e,"stroke-width":0,stroke:e,opacity:0});b.attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var h=b.getBBox();d=this.drawRoundRect(b,h,4);h=this.drawNib(b,h,4);b.toFront();g.push(d,h,b).attr({opacity:0}).toFront();a.toFront();this.checkHoverPos({rect:d,set:g,nib:h});this.globalHoverSet.push(g);
this.globalBlockSet.push(a);d.attrs.y<0&&g.translate(0,1+d.attrs.y*-1);a.node.onmouseover=function(){c.animate({fill:f,stroke:f},200);g.animate({opacity:1},200)}.bind(this);a.node.onmouseout=function(){c.animate({fill:e,stroke:e},200);g.animate({opacity:0},200)}}});
Grafico.HorizontalBarGraph=Class.create(Grafico.BarGraph,{chartDefaults:function(){return{bar:true,horizontalbar:true,plot_padding:0,horizontal_rounded:false,bargraph_lastcolour:false}},setChartSpecificOptions:function(){this.x_padding_left=20+this.longestLabel()*(this.options.font_size/2);this.bar_padding=5;this.bar_width=this.calculateBarHeight();this.step=this.calculateStep();this.graph_width=this.options.width-this.x_padding_right-this.x_padding_left},normalise:function(a){var b=this.makeValueLabels(this.y_label_count);
...
g=this.x_padding_left+f-0.5,h=this.options.bargraph_lastcolour,j=this.options.bargraph_negativecolour;this.datalabel=d;$A(c).each(function(i,k){var l,m=this.options.horizontal_rounded?this.bar_width/2:0,n;l=h&&k===c.length-1?h:i<0?j:b;i=i/this.graph_width*(this.graph_width-f);n=this.paper.rect(g,e,i,this.bar_width,m).attr({fill:l,"stroke-width":0,stroke:l});i<0&&n.attr({width:-n.attrs.width}).translate(i,0);if(m){var p=this.paper.set();bargraph2=this.paper.rect(g,e,i-this.bar_width/2,this.bar_width);
bargraph2.attr({fill:l,"stroke-width":0,stroke:l});n.toFront();p.push(bargraph2,n);i<0&&bargraph2.attr({width:-bargraph2.attrs.width-this.bar_width}).translate(i+this.bar_width/2,0)}if(this.options.datalabels){var q=this.options.hover_color||l,o=this.paper.set();m=this.paper.rect(this.x_padding_left,e,this.graph_width,this.bar_width);d=this.datalabel[k].toString();k=this.paper.text(f+i+this.x_padding_left/2,n.attrs.y-this.options.font_size*1.5,d);m.attr({fill:l,"stroke-width":0,stroke:l,opacity:0});
k.attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});i<0&&k.translate(k.getBBox().width,0);n=k.getBBox();i=this.drawRoundRect(k,n,4);n=this.drawNib(k,n,4);k.toFront();o.push(i,n,k).attr({opacity:0}).toFront();m.toFront();this.checkHoverPos({rect:i,set:o,nib:n});this.globalHoverSet.push(o);this.globalBlockSet.push(m);i.attrs.y<0&&o.translate(0,1+i.attrs.y*-1);m.node.onmouseover=function(){p.animate({fill:q,stroke:q},200);o.animate({opacity:1},200)}.bind(this);m.node.onmouseout=
function(){p.animate({fill:l,stroke:l},200);o.animate({opacity:0},200)}}e+=this.step}.bind(this))},drawFocusHint:function(){var a=this.x_padding_left+10,b=this.options.height-this.y_padding_bottom-2.5;this.paper.path().attr({stroke:this.options.label_color,"stroke-width":2}).moveTo(a,b).lineTo(a-5,b+5).moveTo(a-5,b).lineTo(a-10,b+5)},drawVerticalLabels:function(){var a=this.step/2,b=this.options.label_rotation?{"text-anchor":"end",rotation:this.options.label_rotation,translation:"0 "+this.options.font_size/
2}:{"text-anchor":"end"};this.drawMarkers(this.options.labels.reverse(),[0,-1],this.step,a,[-8,-(this.options.font_size/5)],b)},drawHorizontalLabels:function(){var a=this.graph_width/this.y_label_count,b=this.makeValueLabels(this.y_label_count);if(this.options.vertical_label_unit)for(var c=0;c<b.length;c++)b[c]+=this.options.vertical_label_unit;this.drawMarkers(b,[1,0],a,a,[0,(this.options.font_size+7)*-1])},drawMeanLine:function(a){var b=this.paper.path().attr(this.options.meanline);a=$A(a).inject(0,
function(c,d){return d+c})/a.length;a=this.options.bar?a+this.zero_value*(this.graph_height/this.y_label_count):a;b.moveTo(this.x_padding_left-1+a,this.y_padding_top).lineTo(this.x_padding_left-1+a,this.y_padding_top+this.graph_height)}});
Grafico.SparkLine=Class.create(Grafico.Base,{initialize:function(a,b,c){this.element=a;this.data=b;this.options={highlight:false,stroke_width:1,color:this.makeRandomColour(),width:parseInt(a.getStyle("width"),10),height:parseInt(a.getStyle("height"),10),acceptable_range:false,zeroline:false};Object.extend(this.options,c||{});this.step=this.calculateStep();this.paper=new Raphael(this.element,this.options.width,this.options.height);this.background=this.options.acceptable_range?this.paper.rect(0,this.options.height-
this.normalise(this.options.acceptable_range[1]),this.options.width,this.normalise(this.options.acceptable_range[0])):(this.background=this.paper.rect(0,0,this.options.width,this.options.height));this.background.attr({fill:this.options.background_color,stroke:"none"});this.draw()},calculateStep:function(){return this.options.width/(this.data.length-1)},makeRandomColour:function(){return Raphael.hsb2rgb(Math.random(),1,0.75).hex},normalise:function(a){var b=this.data.min()<0?this.data.max()-this.data.min():
this.data.max();a-=this.data.min()<0?this.data.min():0;return a/b*this.options.height},draw:function(){var a=this.normaliseData(this.data),b;if(this.options.zeroline&&this.data.min()<0){this.options.zeroline=this.options.zeroline===true?{"stroke-width":"1px",stroke:"#BBBBBB"}:this.options.zeroline;b=parseInt(this.options.height-this.normalise(0),10);this.paper.path().attr(this.options.zeroline).moveTo(0,b).lineTo(this.options.width,b)}this.drawLines(this.options.color,a);this.options.highlight&&this.showHighlight(a)},
drawLines:function(a,b){var c=this.paper.path().attr({stroke:a,"stroke-width":this.options.stroke_width}).moveTo(0,this.options.height-b.first()),d=0,e=this.data.min()<0?this.options.stroke_width:0;b.slice(1).each(function(f){d+=this.step;c.lineTo(d,this.options.height-f-e)}.bind(this))},showHighlight:function(a){var b=1+this.options.stroke_width/2,c=this.options.width-b;a=a[this.options.highlight.index||a.length-1]+(b/2).round();var d=this.options.highlight.color||"#f00";if(typeof this.options.highlight.index!==
"undefined")c=this.step*this.options.highlight.index;this.paper.circle(c,this.options.height-a+b/2,b).attr({stroke:false,fill:d})}});
Grafico.SparkBar=Class.create(Grafico.SparkLine,{calculateStep:function(){return this.options.width/this.data.length},drawLines:function(a,b){var c=this.options.bargraph_lastcolour,d=this.step>2?this.step-1:this.step,e=d/2,f=this.normalise(0);b.each(function(g,h){var j;h=c&&h===b.length-1?c:a;j=this.paper.path().attr({stroke:h,"stroke-width":d});j.moveTo(e,this.options.height-g);j.lineTo(e,this.options.height-f);if(g<f)j.attr({stroke:this.options.bargraph_negativecolour||h});e+=this.step}.bind(this))},
