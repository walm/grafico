...
Math.pow(10,c);return(b?a<1.5?1:a<3?2:a<7?5:10:a<=1?1:a<=2?2:a<=5?5:10)*Math.pow(10,c)},roundToOrigin:function(a,b){var c=a;b=b||1;b=Math.pow(10,-b);c=Math.round(a*b)/b;return c>this.min?this.roundToOrigin(a-this.step):c}});
...
k;this.options.odd_horizontal_offset>1&&g.splice(0,this.options.odd_horizontal_offset)}if(this.options.stacked_fill||this.options.area){if(this.options.area){a=this.options.area_opacity?this.options.area_opacity:1.5/this.data_sets.collect(function(l){return l.length}).length;j=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0",opacity:a,"stroke-opacity":0})}else j=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0"});if(!this.hasBaseLine()){g.unshift([g[0][0],h]);g.push([g[g.length-
1][0],h])}}else j=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});$A(g).each(function(l,m){var n=l[0];l=l[1];b instanceof Array?this.drawPlot(m,j,n,l,b[m%b.length],g,d,e,f):this.drawPlot(m,j,n,l,b,g,d,e,f)}.bind(this));if(this.options.area&&this.options.stroke_width>0){i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(l,m){this.drawPlot(m,i,l[0],l[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(i)}if(this.options.datalabels&&
...
6,j=this.step,i=this.options.stacked?this.graph_height/9:this.graph_height/3;b=this.paper.circle(b,c,this.options.marker_size).attr({"stroke-width":"1px",stroke:this.options.background_color,fill:d,opacity:0});d=this.paper.rect(g,h,j,i).attr({fill:d,"stroke-width":0,stroke:d,opacity:0});e=this.options.datalabels?e+": "+f:""+f;e+=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:"";var k=this.paper.set();e=this.paper.text(b.attrs.cx,b.attrs.cy-this.options.font_size*1.5-8,e).attr({"font-size":this.options.font_size,
...
this.bar_padding},calculateStep:function(){this.data_size=this.data_size===1?2:this.data_size;return(this.graph_width-this.options.plot_padding*2-this.bar_padding*2)/(this.data_size-1)},drawPlot:function(a,b,c,d,e,f,g){b=this.options.height-this.y_padding_bottom-this.zero_value*(this.graph_height/this.y_label_count);var h=this.options.bargraph_lastcolor,j=this.options.bargraph_negativecolor||e;c+=this.bar_padding;d=this.options.height-this.y_padding_bottom-d-this.zero_value*(this.graph_height/this.y_label_count);
...
g=this.x_padding_left+f-0.5,h=this.options.bargraph_lastcolor,j=this.options.bargraph_negativecolor||b;this.datalabel=d;$A(c).each(function(i,k){var l,m=this.options.horizontal_rounded?this.bar_width/2:0,n;l=h&&k===c.length-1?h:i<0?j:b;i=i/this.graph_width*(this.graph_width-f);n=this.paper.rect(g,e,i,this.bar_width,m).attr({fill:l,"stroke-width":0,stroke:l});i<0&&n.attr({width:-n.attrs.width}).translate(i,0);if(m){var q=this.paper.set();bargraph2=this.paper.rect(g,e,i-this.bar_width/2,this.bar_width);
bargraph2.attr({fill:l,"stroke-width":0,stroke:l});q.push(bargraph2,n);i<0&&bargraph2.attr({width:-bargraph2.attrs.width-this.bar_width}).translate(i+this.bar_width/2,0)}if(this.options.datalabels){var r=this.options.hover_color||l,o=this.paper.set();k=this.datalabel[k].toString();k=this.paper.text(f+i+this.x_padding_left/2,n.attrs.y-this.options.font_size*1.5,k).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var s=this.paper.rect(this.x_padding_left,e,this.graph_width,
this.bar_width).attr({fill:l,"stroke-width":0,stroke:l,opacity:0}),p=k.getBBox();i<0&&k.translate(p.width,0);i=this.drawRoundRect(k,p,4);p=this.drawNib(k,p,4);o.push(i,p,k).attr({opacity:0});this.checkHoverPos({rect:i,set:o,nib:p});this.globalHoverSet.push(o);this.globalBlockSet.push(s);i.attrs.y<0&&o.translate(0,1-i.attrs.y);s.hover(function(){m?q.animate({fill:r,stroke:r},200):n.animate({fill:r,stroke:over_color},200);o.animate({opacity:1},200)},function(){m?q.animate({fill:l,stroke:l},200):n.animate({fill:l,
