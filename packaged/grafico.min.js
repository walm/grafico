...
draw_axis:true,datalabels:"",hover_color:"",watermark:false,watermark_location:false,hide_empty_label_grid:false,left_padding:false,label_rotation:0,label_max_size:false,focus_hint:true,min:0,max:null};Object.extend(this.options,this.chartDefaults()||{});Object.extend(this.options,c||{});this.element=a;this.data_sets=Object.isArray(b)?new Hash({one:b}):$H(b);if(this.chartDefaults().stacked===true){this.real_data=this.deepCopy(this.data_sets);this.data_sets=this.stackData(this.data_sets)}this.flat_data=
this.data_sets.collect(function(d){return d[1]}).flatten();if(this.hasBaseLine()){this.flat_data.push(this.base_line);this.flat_data=this.flat_data.flatten()}this.normaliser=new Grafico.Normaliser(this.flat_data,this.normaliserOptions());this.label_step=this.normaliser.step;this.range=this.normaliser.range;this.start_value=this.normaliser.start_value;this.zero_value=this.normaliser.zero_value;this.data_size=this.longestDataSetLength();if(c&&c.color){c.colors={};this.data_sets.keys().each(function(d){c.colors[d]=
c.color})}this.options.colors=this.makeRandomColors();this.options.labels=$A($R(1,this.data_size));Object.extend(this.options,this.chartDefaults()||{});Object.extend(this.options,c||{});this.x_padding_left=10+this.paddingLeftOffset();this.x_padding_left+=this.options.vertical_label_unit?6:0;this.x_padding_left=this.options.left_padding?this.options.left_padding:this.x_padding_left;this.x_padding_right=20;this.x_padding=this.x_padding_left+this.x_padding_right;this.y_padding_top=this.options.padding_top;
this.y_padding_bottom=20+this.paddingBottomOffset();this.y_padding=this.y_padding_top+this.y_padding_bottom;this.graph_width=this.options.width-this.x_padding;this.graph_height=this.options.height-this.y_padding;this.step=this.calculateStep();this.y_label_count=(this.range/this.label_step).round();if(isNaN(this.y_label_count)){this.y_label_count=1;this.options.show_vertical_labels=false}if(this.normaliser.min+this.y_label_count*this.normaliser.step<this.normaliser.max)this.y_label_count+=1;this.value_labels=
this.makeValueLabels(this.y_label_count);this.top_value=this.value_labels.last();this.grid_start_offset=-1;this.paper=new Raphael(this.element,this.options.width,this.options.height);this.background=this.paper.rect(this.x_padding_left,this.y_padding_top,this.graph_width,this.graph_height);this.background.attr({fill:this.options.background_color,stroke:"none"});this.options.meanline=this.options.meanline===true?{"stroke-width":"2px",stroke:"#BBBBBB"}:this.options.meanline;this.globalMarkerSet=this.paper.set();
this.globalHoverSet=this.paper.set();this.globalBlockSet=this.paper.set();this.globalAreaLineSet=this.paper.set();this.setChartSpecificOptions();this.draw();this.globalAreaLineSet.toFront();this.globalMarkerSet.toFront();this.globalHoverSet.toFront();this.globalBlockSet.toFront()},normaliserOptions:function(){return{graph_height:parseInt(this.element.getStyle("height"),10)}},hasBaseLine:function(){return false},getNormalizedBaseLine:function(){if(this.normalized_base_line==undefined)this.normalized_base_line=
this.normaliseData(this.base_line);return this.normalized_base_line},getNormalizedRealData:function(){if(this.normalized_real_data==undefined)this.normalized_real_data=this.real_data.collect(function(a){return this.normaliseData(a[1])}.bind(this));return this.normalized_real_data},chartDefaults:function(){},drawPlot:function(){},calculateStep:function(){},getMousePos:function(a){var b=0,c=0;if(!a)a=window.event;if(a.pageX||a.pageY){b=a.pageX;c=a.pageY}else if(a.clientX||a.clientY){b=a.clientX+document.body.scrollLeft-
document.documentElement.scrollLeft;c=a.clientY+document.body.scrollTop-document.documentElement.scrollTop}return{x:b,y:c}},makeRandomColors:function(){var a={},b=1/this.data_sets.size(),c=Math.random();this.data_sets.each(function(d){a[d[0]]=Raphael.hsb2rgb(c,0.85,0.75).hex;c=(c+b)%1});return a},longestDataSetLength:function(){var a=0;this.data_sets.each(function(b){a=b[1].length>a?b[1].length:a});return a},roundValue:function(a,b){b=Math.pow(10,b);a*=b;return a=Math.round(a)/b},roundValues:function(a,
b){return $A(a).collect(function(c){return this.roundValue(c,b)}.bind(this))},paddingLeftOffset:function(){var a=0;if(this.options.show_vertical_labels){a=this.flat_data;a=this.roundValues(a,2);a=$A(a).max(function(b){return b.toString().length});a=a>2?a-1:a;a=a*this.options.font_size}return a},paddingBottomOffset:function(){return this.options.font_size},normalise:function(a){var b=this.start_value===0?this.top_value:this.range;if(b===0)b=1;return a/b*this.graph_height},draw:function(){this.options.grid&&
this.drawGrid();this.options.watermark&&this.drawWatermark();this.options.show_vertical_labels&&this.drawVerticalLabels();this.options.show_horizontal_labels&&this.drawHorizontalLabels();this.options.watermark||this.drawLinesInit(this);this.options.draw_axis&&this.drawAxis();this.start_value!==0&&this.options.focus_hint&&this.drawFocusHint();this.options.meanline&&this.drawMeanLine(this.normaliseData(this.flat_data))},drawLinesInit:function(a){a.data_sets.each(function(b,c){a.drawLines(b[0],a.options.colors[b[0]],
a.normaliseData(b[1]),a.options.datalabels[b[0]],a.element,c)}.bind(a))},drawWatermark:function(){var a=this.options.watermark,b=new Image,c=this;b.onload=function(){var d,e;if(c.options.watermark_location==="middle"){d=(c.graph_width-b.width)/2+c.x_padding_left;e=(c.graph_height-b.height)/2+c.y_padding_top}else{d=c.graph_width-b.width+c.x_padding_left-2;e=c.graph_height-b.height+c.y_padding_top-2}d=c.paper.image(b.src,d,e,b.width,b.height).attr({opacity:"0.4"});c.drawLinesInit(c,c.data);if(c.options.stacked_fill||
c.options.area)d.toFront()};b.src=a.src||a},drawGrid:function(){var a=this.paper.path().attr({stroke:this.options.grid_color}),b,c,d;if(this.options.show_horizontal_grid){b=this.graph_height+this.y_padding_top;for(d=0;d<this.y_label_count+1;d++){a.moveTo(this.x_padding_left-0.5,parseInt(b,10)+0.5);a.lineTo(this.x_padding_left+this.graph_width-0.5,parseInt(b,10)+0.5);b-=this.graph_height/this.y_label_count}}if(this.options.show_vertical_grid){b=this.x_padding_left+this.options.plot_padding+this.grid_start_offset;
c=this.options.labels.length;for(d=0;d<c;d++){if(this.options.hide_empty_label_grid===true&&this.options.labels[d]!==""||this.options.hide_empty_label_grid===false){a.moveTo(parseInt(b,10),this.y_padding_top);a.lineTo(parseInt(b,10),this.y_padding_top+this.graph_height)}b+=this.step}}},drawLines:function(a,b,c,d,e,f){var g=this.calculateCoords(c),h=this.graph_height+this.y_padding_top,i,j,k;if(this.options.start_at_zero===false){k=0;$A(g).each(function(l){l[1]===h&&k++});this.options.odd_horizontal_offset=
k;this.options.odd_horizontal_offset>1&&g.splice(0,this.options.odd_horizontal_offset)}if(this.options.stacked_fill||this.options.area){if(this.options.area){a=this.options.area_opacity?this.options.area_opacity:1.5/this.data_sets.collect(function(l){return l.length}).length;i=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0",opacity:a,"stroke-opacity":0})}else i=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0"});if(!this.hasBaseLine()){g.unshift([g[0][0],h]);g.push([g[g.length-
1][0],h])}}else i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});$A(g).each(function(l,m){var n=l[0];l=l[1];b instanceof Array?this.drawPlot(m,i,n,l,b[m%b.length],g,d,e,f):this.drawPlot(m,i,n,l,b,g,d,e,f)}.bind(this));if(this.options.area&&this.options.stroke_width>0){j=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(l,m){this.drawPlot(m,j,l[0],l[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(j)}if(this.options.datalabels&&
this.options.draw_hovers){this.drawHover(i,d,e,b);this.globalHoverSet.toFront()}},calculateCoords:function(a){var b=this.x_padding_left+this.options.plot_padding-this.step,c=this.graph_height+this.y_padding_top+this.normalise(this.start_value);a=$A(a).collect(function(f){b+=this.step;return[b,c-f]}.bind(this));if(!this.hasBaseLine())return a;b+=this.step;for(var d=this.getNormalizedBaseLine(),e=d.length-1;e>=0;e--){b-=this.step;a.push([b,c-d[e]])}return a},drawFocusHint:function(){var a=this.x_padding_left+
...
10)-0.5,parseInt(this.y_padding_top,10))},makeValueLabels:function(a){for(var b=this.label_step,c=this.start_value,d=[],e=0;e<a;e++){c=this.roundValue(c+b,3);d.push(c)}return d},drawMarkers:function(a,b,c,d,e,f){function g(m){return m*b[0]}function h(m){return m*b[1]}var i=parseInt(this.x_padding_left,10)-0.5+g(d),j=this.options.height-this.y_padding_bottom+h(d),k=this.paper.path().attr({stroke:this.options.label_color}),l={font:this.options.font_size+'px "Arial"',stroke:"none",fill:this.options.label_color};
Object.extend(l,f||{});a.each(function(m){if(this.options.draw_axis&&(this.options.hide_empty_label_grid===true&&m!==""||this.options.hide_empty_label_grid===false)&&this.options.show_ticks){k.moveTo(parseInt(i,10),parseInt(j,10)+0.5);k.lineTo(parseInt(i,10)+h(5),parseInt(j,10)+0.5+g(5))}this.paper.text(i+e[0],j-2-e[1],m.toString()).attr(l);i+=g(c);j+=h(c)}.bind(this))},drawVerticalLabels:function(){for(var a=this.graph_height/this.y_label_count,b=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:
...
6,i=this.step,j=this.options.stacked?this.graph_height/9:this.graph_height/3;b=this.paper.circle(b,c,this.options.marker_size).attr({"stroke-width":"1px",stroke:this.options.background_color,fill:d,opacity:0});d=this.paper.rect(g,h,i,j).attr({fill:d,"stroke-width":0,stroke:d,opacity:0});e=this.options.datalabels?e+": "+f:""+f;e+=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:"";var k=this.paper.set();e=this.paper.text(b.attrs.cx,b.attrs.cy-this.options.font_size*1.5-8,e).attr({"font-size":this.options.font_size,
fill:this.options.hover_text_color,opacity:1});f=e.getBBox();c=this.drawRoundRect(e,f,4);f=this.drawNib(e,f,4);k.push(b,c,f,e).attr({opacity:0});this.checkHoverPos({rect:c,set:k,marker:b,nib:f,textpadding:4});this.globalHoverSet.push(k);this.globalBlockSet.push(d);d.hover(function(){k.animate({opacity:1},200)},function(){k.animate({opacity:0},200)})}}});
Grafico.AreaGraph=Class.create(Grafico.LineGraph,{chartDefaults:function(){return{area:true,area_opacity:false,stroke_width:0,curve_amount:10}},drawPlot:function(a,b,c,d,e,f,g,h,i,j){var k=this.options.area||this.options.stacked_fill;if(!j)if(k===true){if(a!==0&&a!==f.length-1)if(this.options.markers==="circle")this.drawGraphMarkers(a,c,d,e,g,h);else this.options.markers==="value"&&this.drawGraphValueMarkers(a,c,d,e,g,h,i)}else if(this.options.markers==="circle")this.drawGraphMarkers(a,c,d,e,g,h);
else this.options.markers==="value"&&this.drawGraphValueMarkers(a,c,d,e,g,h,i);c-=0.5;if(a===0)return this.startPlot(b,c,d,e);if(this.options.curve_amount&&a>1&&a<f.length-1)b.cplineTo(c,d,this.options.curve_amount);else this.options.curve_amount&&!k&&1?b.cplineTo(c,d,this.options.curve_amount):b.lineTo(c,d)}});
...
a.moveTo(parseInt(this.x_padding_left+this.graph_width,10)-0.5,this.y_padding_top);a.lineTo(parseInt(this.x_padding_left+this.graph_width,10)-0.5,this.y_padding_top+this.graph_height)},drawGraphValueMarkers:function(a,b,c,d,e){var f=this.options.hover_color||e,g=this.paper.set(),h=this.paper.rect(a-this.bar_width/2,this.y_padding_top,this.bar_width,this.options.height);d=d?d[b].toString():"";a=this.paper.text(c.attrs.x+this.bar_width/2,c.attrs.y-this.options.font_size*1.5,d);h.attr({fill:e,"stroke-width":0,
stroke:e,opacity:0});a.attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var i=a.getBBox(),j=this.drawRoundRect(a,i,4);i=this.drawNib(a,i,4);g.push(j,i,a).attr({opacity:0});this.checkHoverPos({rect:j,set:g,nib:i});this.globalHoverSet.push(g);this.globalBlockSet.push(h);j.attrs.y<0&&g.translate(0,1+j.attrs.y*-1);var k=this.options.onclick&&Object.isFunction(this.options.onclick);if(k){var l=this.options.labels,m=this.options.onclick;h.click(function(){m(b,l[b],
d)})}var n=this.element;h.hover(function(){c.animate({fill:f,stroke:f},200);g.animate({opacity:1},200);k&&n.setStyle({cursor:"pointer"})},function(){c.animate({fill:e,stroke:e},200);g.animate({opacity:0},200);k&&n.setStyle({cursor:"default"})})}});
...
b[c+1][d];return a},calculateCoords:function(a){var b=this.x_padding_left+this.options.plot_padding-this.step,c=this.graph_height+this.y_padding_top+this.normalise(this.start_value);a=$A(a).collect(function(f){b+=this.step;return[b,c-f]}.bind(this));b+=this.step;for(var d=this.getNormalizedBaseLine(),e=d.length-1;e>=0;e--){b-=this.step;a.push([b,c-d[e]])}return a},drawLines:function(a,b,c,d,e,f){var g=this.calculateCoords(c),h;h=this.paper.path().attr({stroke:b,"stroke-width":0});$A(g).each(function(i,
j){this.drawPlot(j,h,i[0],i[1],b,g,d,e,f)}.bind(this));if(this.options.datalabels&&this.options.draw_hovers){this.drawHover(h,d,e,b);this.globalHoverSet.toFront()}},drawPlot:function(a,b,c,d,e,f,g){b=this.options.height-this.y_padding_bottom-this.zero_value*(this.graph_height/this.y_label_count);c+=this.bar_padding;d=this.options.height-this.y_padding_bottom-d-this.zero_value*(this.graph_height/this.y_label_count);f={fill:e,"stroke-width":0,stroke:this.options.color2,opacity:this.options.opacity};
b=this.paper.rect(c-this.bar_width/2,b,this.bar_width,d).attr(f);d<0?b.attr({height:-b.attrs.height}):b.translate(0,-d);this.options.datalabels&&this.drawGraphValueMarkers(c,a,b,g,e);this.options.count++},drawGraphValueMarkers:function(a,b,c,d,e){a=a-this.bar_width/2;var f=c.attrs.y,g=this.bar_width,h=c.attrs.height,i=this.options.hover_color||e,j=this.paper.set();f=this.paper.rect(a,f,g,h);d=d&&d.length>b?d[b].toString():"";a=this.paper.text(c.attrs.x+this.bar_width/2,c.attrs.y-this.options.font_size*
1.5,d);f.attr({fill:e,"stroke-width":0,stroke:e,opacity:0});a.attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});h=a.getBBox();g=this.drawRoundRect(a,h,4);h=this.drawNib(a,h,4);j.push(g,h,a).attr({opacity:0});this.checkHoverPos({rect:g,set:j,nib:h});this.globalHoverSet.push(j);this.globalBlockSet.push(f);g.attrs.y<0&&j.translate(0,1+g.attrs.y*-1);var k=this.options.onclick&&Object.isFunction(this.options.onclick);if(k){var l=this.options.labels,m=this.options.onclick;
f.click(function(){m(b,l[b],d)})}var n=this.element;f.hover(function(){c.animate({fill:i},200);j.animate({opacity:1},200);k&&n.setStyle({cursor:"pointer"})},function(){c.animate({fill:e},200);j.animate({opacity:0},200);k&&n.setStyle({cursor:"default"})})}});
...
g=this.x_padding_left+f-0.5,h=this.options.bargraph_lastcolor,i=this.options.bargraph_negativecolor||b;this.datalabel=d;$A(c).each(function(j,k){var l,m=this.options.horizontal_rounded?this.bar_width/2:0,n;l=h&&k===c.length-1?h:j<0?i:b;j=j/this.graph_width*(this.graph_width-f);n=this.paper.rect(g,e,j,this.bar_width,m).attr({fill:l,"stroke-width":0,stroke:l});j<0&&n.attr({width:-n.attrs.width}).translate(j,0);if(m){var q=this.paper.set();bargraph2=this.paper.rect(g,e,j-this.bar_width/2,this.bar_width);
bargraph2.attr({fill:l,"stroke-width":0,stroke:l});q.push(bargraph2,n);j<0&&bargraph2.attr({width:-bargraph2.attrs.width-this.bar_width}).translate(j+this.bar_width/2,0)}if(this.options.datalabels){var r=this.options.hover_color||l,o=this.paper.set();k=this.datalabel[k].toString();k=this.paper.text(f+j+this.x_padding_left/2,n.attrs.y-this.options.font_size*1.5,k).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var s=this.paper.rect(this.x_padding_left,e,this.graph_width,
this.bar_width).attr({fill:l,"stroke-width":0,stroke:l,opacity:0}),p=k.getBBox();j<0&&k.translate(p.width,0);j=this.drawRoundRect(k,p,4);p=this.drawNib(k,p,4);o.push(j,p,k).attr({opacity:0});this.checkHoverPos({rect:j,set:o,nib:p});this.globalHoverSet.push(o);this.globalBlockSet.push(s);j.attrs.y<0&&o.translate(0,1-j.attrs.y);s.hover(function(){m?q.animate({fill:r,stroke:r},200):n.animate({fill:r,stroke:over_color},200);o.animate({opacity:1},200)},function(){m?q.animate({fill:l,stroke:l},200):n.animate({fill:l,
...
Grafico.SparkBar=Class.create(Grafico.SparkLine,{calculateStep:function(){return this.options.width/this.data.length},drawLines:function(a,b){var c=this.options.bargraph_lastcolor,d=this.step>2?this.step-1:this.step,e=d/2,f=this.normalise(0);b.each(function(g,h){var i;h=c&&h===b.length-1?c:a;i=this.paper.path().attr({stroke:h,"stroke-width":d});i.moveTo(e,this.options.height-g);i.lineTo(e,this.options.height-f);if(g<f)i.attr({stroke:this.options.bargraph_negativecolor||h});e+=this.step}.bind(this))},
showHighlight:function(){}});
Grafico.SparkArea=Class.create(Grafico.SparkLine,{drawLines:function(a,b){var c=a,d=a,e=0.2,f=this.normalise(0);if(typeof a=="object"){c=a[1];d=a[0];e=1}var g=this.paper.path().attr({fill:c,stroke:c,"stroke-width":0,"stroke-opacity":0,opacity:e}).moveTo(0,this.options.height-f).lineTo(0,this.options.height-b.first()),h=this.paper.path().attr({stroke:d,"stroke-width":this.options.stroke_width}).moveTo(0,this.options.height-b.first()),i=0;b.slice(1).each(function(j){i+=this.step;g.lineTo(i,this.options.height-
j);h.lineTo(i,this.options.height-j)}.bind(this));g.lineTo(i,this.options.height-f)}});
