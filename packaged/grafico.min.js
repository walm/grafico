...
parseInt(b,10)+0.5);b-=this.graph_height/this.y_label_count}b=this.x_padding_left+this.options.plot_padding+this.grid_start_offset;c=this.options.labels.length;for(d=0;d<c;d++){if(this.options.hide_empty_label_grid===true&&this.options.labels[d]!==""||this.options.hide_empty_label_grid===false){a.moveTo(parseInt(b,10),this.y_padding_top);a.lineTo(parseInt(b,10),this.y_padding_top+this.graph_height)}b+=this.step}},drawLines:function(a,b,c,d,e,f){var g=this.calculateCoords(c),h=this.graph_height+this.y_padding_top,
i,j,l;if(this.options.start_at_zero===false){l=0;$A(g).each(function(k){k[1]===h&&l++});this.options.odd_horizontal_offset=l;this.options.odd_horizontal_offset>1&&g.splice(0,this.options.odd_horizontal_offset)}if(this.options.stacked_fill||this.options.area){if(this.options.area){a=this.options.area_opacity?this.options.area_opacity:1.5/this.data_sets.collect(function(k){return k.length}).length;i=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0",opacity:a,"stroke-opacity":0})}else i=this.paper.path().attr({stroke:b,
fill:b,"stroke-width":"0"});g.unshift([g[0][0],h]);g.push([g[g.length-1][0],h])}else i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});$A(g).each(function(k,m){this.drawPlot(m,i,k[0],k[1],b,g,d,e,f)}.bind(this));if(this.options.area&&this.options.stroke_width>0){j=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(k,m){this.drawPlot(m,j,k[0],k[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(j)}if(this.options.datalabels&&
