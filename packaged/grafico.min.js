...
drawWatermark:function(){var a=this.options.watermark,b=new Image,c=this;b.onload=function(){var d,e;if(c.options.watermark_location==="middle"){d=(c.graph_width-b.width)/2+c.x_padding_left;e=(c.graph_height-b.height)/2+c.y_padding_top}else{d=c.graph_width-b.width+c.x_padding_left-2;e=c.graph_height-b.height+c.y_padding_top-2}d=c.paper.image(b.src,d,e,b.width,b.height).attr({opacity:"0.4"});c.drawLinesInit(c,c.data);if(c.options.stacked_fill||c.options.area)d.toFront()};b.src=a.src||a},drawGrid:function(){var a=
this.paper.path().attr({stroke:this.options.grid_color}),b,c;if(this.options.show_horizontal_grid){b=this.graph_height+this.y_padding_top;for(var d=0;d<this.y_label_count+1;d++){a.moveTo(this.x_padding_left-0.5,parseInt(b,10)+0.5);a.lineTo(this.x_padding_left+this.graph_width-0.5,parseInt(b,10)+0.5);b-=this.graph_height/this.y_label_count}}if(this.options.show_vertical_grid){b=this.x_padding_left+this.options.plot_padding+this.grid_start_offset;c=this.options.labels.length;for(d=0;d<c;d++){if(this.options.hide_empty_label_grid===
true&&this.options.labels[d]!==""||this.options.hide_empty_label_grid===false){a.moveTo(parseInt(b,10),this.y_padding_top);a.lineTo(parseInt(b,10),this.y_padding_top+this.graph_height)}b+=this.step}}},drawLines:function(a,b,c,d,e,f){var g=this.calculateCoords(c),h=this.graph_height+this.y_padding_top,i,j,k;if(this.options.start_at_zero===false){k=0;$A(g).each(function(l){l[1]===h&&k++});this.options.odd_horizontal_offset=k;this.options.odd_horizontal_offset>1&&g.splice(0,this.options.odd_horizontal_offset)}if(this.options.stacked_fill||
this.options.area){if(this.options.area){a=this.options.area_opacity?this.options.area_opacity:1.5/this.data_sets.collect(function(l){return l.length}).length;i=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0",opacity:a,"stroke-opacity":0})}else i=this.paper.path().attr({stroke:b,fill:b,"stroke-width":"0"});if(!this.hasBaseLine()){g.unshift([g[0][0],h]);g.push([g[g.length-1][0],h])}}else i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});$A(g).each(function(l,
m){var n=l[0];l=l[1];b instanceof Array?this.drawPlot(m,i,n,l,b[m%b.length],g,d,e,f):this.drawPlot(m,i,n,l,b,g,d,e,f)}.bind(this));if(this.options.area&&this.options.stroke_width>0){j=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(l,m){this.drawPlot(m,j,l[0],l[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(j)}if(this.options.datalabels&&this.options.draw_hovers){this.drawHover(i,d,e,b);this.globalHoverSet.toFront()}},
...
this.roundValue(c+b,3);d.push(c)}return d},drawMarkers:function(a,b,c,d,e,f){function g(m){return m*b[0]}function h(m){return m*b[1]}var i=parseInt(this.x_padding_left,10)-0.5+g(d),j=this.options.height-this.y_padding_bottom+h(d),k=this.paper.path().attr({stroke:this.options.label_color}),l={font:this.options.font_size+'px "Arial"',stroke:"none",fill:this.options.label_color};Object.extend(l,f||{});a.each(function(m){if(this.options.draw_axis&&(this.options.hide_empty_label_grid===true&&m!==""||this.options.hide_empty_label_grid===
...
d,h=this.paper.set();b=this.paper.text(a.attrs.x,a.attrs.y-this.options.font_size*1.5-4,b).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var i=b.getBBox(),j=this.drawRoundRect(b,i,4);h.push(j,b).attr({opacity:0});this.checkHoverPos({rect:j,set:h});this.globalHoverSet.push(h);a.hover(function(k){f==="fill"?a.animate({fill:g,stroke:g},200):a.animate({stroke:g},200);k=e.getMousePos(k);h[0].attr({x:k.x-i.width/2-4-c.offsetLeft,y:k.y-i.height/2-e.options.font_size*
1.5-4-c.offsetTop,opacity:1});h[1].attr({x:k.x-c.offsetLeft,y:k.y-e.options.font_size*1.5-c.offsetTop,opacity:1});a.mousemove(function(l){l=e.getMousePos(l);h[0].attr({x:l.x-i.width/2-4-c.offsetLeft,y:l.y-i.height/2-e.options.font_size*1.5-4-c.offsetTop,opacity:1});h[1].attr({x:l.x-c.offsetLeft,y:l.y-e.options.font_size*1.5-c.offsetTop,opacity:1});e.checkHoverPos(j,h)})},function(){f==="fill"?a.animate({fill:d,stroke:d},200):a.animate({stroke:d},200);h.attr({opacity:0})})},checkHoverPos:function(a){var b,
...
g=this.x_padding_left+f-0.5,h=this.options.bargraph_lastcolor,i=this.options.bargraph_negativecolor;this.datalabel=d;$A(c).each(function(j,k){var l,m=this.options.horizontal_rounded?this.bar_width/2:0,n;l=h&&k===c.length-1?h:j<0?i:b;j=j/this.graph_width*(this.graph_width-f);n=this.paper.rect(g,e,j,this.bar_width,m).attr({fill:l,"stroke-width":0,stroke:l});j<0&&n.attr({width:-n.attrs.width}).translate(j,0);if(m){var p=this.paper.set();bargraph2=this.paper.rect(g,e,j-this.bar_width/2,this.bar_width);
bargraph2.attr({fill:l,"stroke-width":0,stroke:l});p.push(bargraph2,n);j<0&&bargraph2.attr({width:-bargraph2.attrs.width-this.bar_width}).translate(j+this.bar_width/2,0)}if(this.options.datalabels){var q=this.options.hover_color||l,o=this.paper.set();k=this.datalabel[k].toString();text=this.paper.text(f+j+this.x_padding_left/2,n.attrs.y-this.options.font_size*1.5,k).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});hoverbar=this.paper.rect(this.x_padding_left,
e,this.graph_width,this.bar_width).attr({fill:l,"stroke-width":0,stroke:l,opacity:0});textbox=text.getBBox();j<0&&text.translate(textbox.width,0);j=this.drawRoundRect(text,textbox,4);n=this.drawNib(text,textbox,4);o.push(j,n,text).attr({opacity:0});this.checkHoverPos({rect:j,set:o,nib:n});this.globalHoverSet.push(o);this.globalBlockSet.push(hoverbar);j.attrs.y<0&&o.translate(0,1+j.attrs.y*-1);hoverbar.hover(function(){p.animate({fill:q,stroke:q},200);o.animate({opacity:1},200)},function(){p.animate({fill:l,
